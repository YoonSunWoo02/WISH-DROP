    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('친구 요청을 보냈어요! 상대방이 수락하면 친구가 돼요.')),
        );
      }
    } catch (e) {
      if (mounted) {
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(
                          formatNicknameCode(user.nickname, user.friendCode),
                          style: const TextStyle(fontSize: 12),
                        ),
                        trailing: switch (status) {
                          'accepted' => const Chip(label: Text('친구 ✓')),
                          'pending' => const Chip(label: Text('요청 중')),
                          _ => ElevatedButton(
